...
class conv2d():
    def __init__(self, input_shape, filter_shape, strides, padding='SAME'):
        ...
        Weight = np.sqrt(2. / (self.f_H * self.f_W * self.in_D)) * np.random.randn(self.out_D, self.f_H, self.f_W,
                                                                                   self.in_D)
    ...
    def forward_propagate(self, X):
        self.X_col = im2col(X, [self.f_H, self.f_W], pad=[self.pad_H, self.pad_W],
                            stride=[self.stride_H, self.stride_W])  # shape=(f_H*f_W*in_D,out_H*out_W*BS)
        out = np.matmul(self.W_col, self.X_col) + self.b  # shape=(out_D,out_H*out_W*BS)
        out = out.reshape(self.out_D, self.out_H, self.out_W,
...
class max_pooling():
    ...
    def forward_propagate(self, X):
        X_col = im2col(X, [self.f_H, self.f_W], pad=[self.pad_H, self.pad_W],
                       stride=[self.stride_H, self.stride_W])  # shape=(in_D*f_H*f_W,out_H*out_W*BS)
        X_col_channel = X_col.reshape(self.in_D, self.f_H * self.f_W,
                                      self.out_H * self.out_W * self.BS)  # shape=(in_D,f_H*f_W,out_H*out_W*BS)
        cord_1 = np.argmax(X_col_channel, axis=1).reshape(
            self.in_D * self.out_H * self.out_W * self.BS)  # shape=(in_D*out_H*out_W*BS)
        ...
        out = out.reshape(self.in_D, self.out_H, self.out_W,
                          self.BS)  # shape=(in_D,out_H*out_W*BS)->(in_D,out_H,out_W,BS)
        out = out.transpose(3, 0, 1, 2)  # shape=(BS,out_D,out_H,out_W)
        return out

    def back_propagate(self, dout):
        dout_reshaped = dout.transpose(1, 2, 3, 0).reshape(self.in_D, 1,
                                                           self.out_H * self.out_W * self.BS)  # shape=(BS,in_D,out_H,out_W)->(in_D,out_H*out_W*BS)

        din_col = (self.iomat * dout_reshaped).reshape(self.in_D * self.f_H * self.f_W,
...
class full_connect():
    ...
    def forward_propagate(self, input):
        self.input = input
        output = input @ self.W + self.b  # shape=(BS,output_len)
        return output
